...
function canonicalize(rawData) {
  ...
  for (const [rep, partnerData] of Object.entries(rawData)) {
    const cr = REPORTER_CANONICAL[rep] || rep;
...
// ── 4. PRICE TIMELINE (TRUE GLOBAL AVERAGE) ─────────────────────────────────
function buildGlobalPrices(qtyCsvPath, valCsvPath) {
  const result = {};
  const qtyTotals = { '656': {}, '657': {} };
  const valTotals = { '656': {}, '657': {} };

  YEARS.forEach(y => {
    qtyTotals['656'][y] = 0; qtyTotals['657'][y] = 0;
    valTotals['656'][y] = 0; valTotals['657'][y] = 0;
  });

  function processCSV(csvPath, targetTotals, validElems) {
    const text = fs.readFileSync(csvPath, 'utf8');
    const lines = text.split('\n');
    const hdr = parseCSVLine(lines[0]);
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (item !== '656' && item !== '657') continue;
      if (!validElems.includes(elem)) continue;

      YEARS.forEach(y => {
        const col = yearCol[y];
        if (col !== undefined && col < f.length) {
          const v = parseFloat(f[col]);
          if (!isNaN(v) && v > 0) {
            targetTotals[item][y] += v;
          }
        }
      });
    }
  ...
  processCSV(qtyCsvPath, qtyTotals, ['5910', '5610']);
  processCSV(valCsvPath, valTotals, ['5922', '5622']);

  for (const yr of YEARS) {
    const qR = qtyTotals['656'][yr];
    const vR = valTotals['656'][yr];
    const qP = qtyTotals['657'][yr];
    const vP = valTotals['657'][yr];

    const raw = qR > 0 ? Math.round((vR * 1000) / qR) : null;
    const processed = qP > 0 ? Math.round((vP * 1000) / qP) : null;
...
const PT = buildGlobalPrices(QTY_CSV, VAL_CSV);

// ── WRITE OUTPUT JSON ───────────────────────────────────────────────────────
const masterData = {
  TRANSFORMATION_INDEX_BY_YEAR: TI_BY_YEAR,
  TRADE_QUANTITY_BY_YEAR: TQ_BY_YEAR,
  TRADE_VALUE_BY_YEAR: TV_BY_YEAR,
  TRADE_QTY_RAW_BY_YEAR: TQ_RAW_BY_YEAR,
  TRADE_QTY_PROC_BY_YEAR: TQ_PROC_BY_YEAR,
  PROCESSED_SHARE_BY_YEAR: PS_BY_YEAR,
  PRICE_TIMELINE: PT,
  TRADE_QUANTITY: TQ_BY_YEAR[2023] || {},
  TRADE_VALUE: TV_BY_YEAR[2023] || {},
